 ...
 *
 * Modified by humanizer-pro (2026-09-17): adjusted require path to load the
 * relocated engine at skills/humanizer-pro/scripts/lib/en-detector/index.js,
 * and points CATEGORIES.md / the README count-site excerpt at copied
 * fixtures under tests/fixtures/en-upstream/ (this test depends on
 * repo-specific docs that do not exist in humanizer-pro; _sources/ is
 * gitignored so those upstream files cannot be read directly at runtime).
 * Upstream: conorbronsdon/avoid-ai-writing@7a2c7d11 detector/categories.test.js
 */
...
const md = fs.readFileSync(path.join(__dirname, 'fixtures/en-upstream/CATEGORIES.md'), 'utf8');
...
const COUNT_SITES = [
  ['fixtures/en-upstream/README-excerpt.md', /engine implements (\d+) `type` categories/],
  ['fixtures/en-upstream/CATEGORIES.md', /engine exposes (\d+) issue `type`s/],
  ['fixtures/en-upstream/CATEGORIES.md', /the engine's \*\*(\d+) `type`s\*\*/],
];
