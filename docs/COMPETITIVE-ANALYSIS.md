...
| IMP-05 | Packaged distribution: npm bin plus host manifests. Later redesigned: the project owner objected to per-vendor manifest files at the repo root, so `.claude-plugin/`, `.codex-plugin/` and `agents/openai.yaml` were removed in favour of `tools/install.js`, a single neutral installer | `package.json`, `tools/install.js` | M | A4 (amanmaqsood row 2) | `npm pack` produces a tarball whose CLI runs `detect.js` and `validate.js`; `tools/install.js` copies `skills/humanizer-pro/` to any target directory and verifies the copy by file count and byte count (`tests/install.test.js`) | Manifest shapes are host-specific and drift; the installer replaces them rather than adding to them. Do not add dependencies |
...
| Tests and CI | amanmaqsood (Node 18/20/22 matrix) | **now us**. Tests went 114 to 226, IMP-04's benchmark passes 16/16, and the CI matrix is green on its first run: Node 18, 20 and 22 on Linux plus Node 20 on Windows (https://github.com/derabia/humanizer-pro/actions/runs/35500327096) |
...
| Distribution and packaging | amanmaqsood (npm plus 4 manifests) | still amanmaqsood. We removed our three per-vendor manifests (the project owner objected to them) in favour of `tools/install.js`, a single neutral installer, and npm `bin` entries stay. `npm pack --dry-run` passes and the tarball CLIs run, nothing is published, and no host loader has run against `skills/humanizer-pro/` |
